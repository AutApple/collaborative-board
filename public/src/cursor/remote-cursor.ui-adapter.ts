...
export class RemoteCursorUIAdapter {
  ...
  constructor(
    ...
    cursorContainerListId: string = 'cursors-layer',
    private cursorClassName: string = 'remote-cursor',
  ) {
    this.cursorContainer = document.getElementById(cursorContainerListId) as HTMLDivElement;

    if (!this.cursorContainer)
      throw new Error("Can't find a cursor container element. Does id spelled correctly?");
  }
