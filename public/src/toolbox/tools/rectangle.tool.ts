...
export class RectangleTool extends BaseTool {
  ...
  public startConstructing(worldCoords: Vec2, strokeData: StrokeData): ToolResult | null {
    ...
    return new ToolResult()
      .addBoardAction((board) => board.appendElement(this.constructingSquarePointer!))
      .addRenderBoardEmit();
  }
  public stepConstructing(worldCoords: Vec2): ToolResult | null {
    if (!this.isConstructing()) return null;
    this.constructingSquarePointer!.setVertices(
      this.getRectVertices(this.constructingSquarePointer!.getPosition(), worldCoords),
    );
