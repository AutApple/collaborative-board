...
class BoardAction {
  ...
  private static findInverseMutation(
    mutation: BaseBoardMutation,
    mutationList: BoardMutationList,
  ): BaseBoardMutation | null {
    ...
    switch (mutation.type) {
      ...
      case BoardMutationType.Remove:
        ...
        for (let i = 0; i < mutationList.length; i++) {
          if (
            mutationList[i]!.id === mutation.id &&
            mutationList[i]!.type === BoardMutationType.Create
          )
            createMutation = mutationList[i] as CreateBoardMutation;
          if (
            mutationList[i]!.id === mutation.id &&
            mutationList[i]!.type === BoardMutationType.Update &&
            i > maxIdx
          ) {
  ...
  public findInverseAction(actionList: BoardAction[]): BoardAction | null {
    ...
    for (const action of actionList) mutationList.push(...action.getMutations());
