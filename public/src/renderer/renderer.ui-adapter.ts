...
export class RendererUiAdapter {
    constructor(private document: Document, semanticEventBus: EventBus<SemanticEventMap>, saveButtonId: string = 'top-panel-save-button') {
        const saveButton = document.getElementById(saveButtonId) as HTMLButtonElement;
        saveButton.addEventListener('click', _ => { semanticEventBus.emit(SemanticEvents.RendererExportBoard, {}); });
    }
