...
export class NetworkingEventHandler extends BaseEventHandler {
    ...
    public onHandshake(cursorWorldCoords: XY) {        
        ...
        const cursor = { clientId: this.client.getClientId(), worldCoords: cursorWorldCoords };
        
        this.appContext.cursorMap.addCursor(cursor);
        this.socket.broadcast.emit(ServerBoardEvents.ClientConnected, this.client.getClientId(), cursor);
