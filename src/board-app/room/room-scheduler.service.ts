export class RoomSchedulerService {
    ...
    public schedule(roomId: string, timerId: string, delayMs: number = 10000, callback: (roomId: string) => Promise<void>) {
    ...
    public scheduleRegular(roomId: string, timerId: string, intervalMs: number, callback: (roomId: string) => Promise<void>) {
        const fullId = `${roomId}:${timerId}`;
        if (this.timers.has(fullId)) return;
